...
class Solution {
    void matchPairs(int n, char nuts[], char bolts[]) {
        // code here
        int[] nutsAscii = new int[n], boltsAscii = new int[n];
        for(int  i = 0; i < n; i++) {
            nutsAscii[i] = (int)nuts[i];
            boltsAscii[i] = (int)bolts[i];
        }
        Arrays.sort(nutsAscii);
        Arrays.sort(boltsAscii);
        for(int  i = 0; i < n; i++) {
            nuts[i] = (char)nutsAscii[i];
            bolts[i] = (char)boltsAscii[i];
        }
    }
}
